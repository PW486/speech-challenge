...
def run_graph(wav_dir, labels, input_layer_name, output_layer_name,
  ...
  with tf.Session() as sess:
    ...
    for idx, wav_path in enumerate(glob.glob(wav_dir + "/*.wav")):
      ...
      top_k = predictions.argsort()[-num_top_predictions:][::-1][0]

      print('%d-%s [%s] (score = %.5f)' % (idx+1, wav_path.split('/')[-1], labels[top_k].replace('_',''), predictions[top_k]))
      file.write('%s,%s\n' % (wav_path.split('/')[-1], labels[top_k].replace('_','')))
