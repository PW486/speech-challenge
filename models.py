...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  elif model_architecture == 'cnn_trad_fpool3':
    return create_cnn_trad_fpool3_model(fingerprint_input, model_settings,
                                        is_training)
  elif model_architecture == 'esc_cnn':
    return create_esc_cnn_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'deep_cnn':
    return create_deep_cnn_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'deeep_cnn':
    return create_deeep_cnn_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'deeeep_cnn':
    return create_deeeep_cnn_model(fingerprint_input, model_settings, is_training)
  else:
...
def create_cnn_trad_fpool3_model(fingerprint_input, model_settings, is_training):
  """Builds a standard convolutional model.
...
def create_esc_cnn_model(fingerprint_input, model_settings, is_training):
  """Builds a standard convolutional model.
  ...
  fingerprint_4d = tf.reshape(fingerprint_input,
                              [-1, input_time_size, input_frequency_size, 1])
  first_filter_width = 6
  first_filter_height = 57
  ...
  if is_training:
    ...
    first_dropout = first_relu
  first_max_pool = tf.nn.max_pool(first_dropout, [1, 4, 3, 1], [1, 1, 3, 1], 'SAME')
  second_filter_width = 3
  second_filter_height = 1
  second_filter_count = 80
  second_weights = tf.Variable(
      tf.truncated_normal(
          [
              second_filter_height, second_filter_width, first_filter_count,
  ...
  if is_training:
    ...
    second_dropout = second_relu
  second_max_pool = tf.nn.max_pool(second_dropout, [1, 1, 3, 1], [1, 1, 3, 1], 'SAME')
...
def create_deep_cnn_model(fingerprint_input, model_settings, is_training):
  """Builds a standard convolutional model.
  ...
  fingerprint_4d = tf.reshape(fingerprint_input,
                              [-1, input_time_size, input_frequency_size, 1])
  first_filter_width = 12
  first_filter_height = 64
  first_filter_count = 80
  first_weights = tf.Variable(
      tf.truncated_normal(
          [first_filter_height, first_filter_width, 1, first_filter_count],
  ...
  second_filter_width = 6
  second_filter_height = 32
  second_filter_count = 80
  second_weights = tf.Variable(
      tf.truncated_normal(
          [
  ...
  third_filter_height = 16
  third_filter_count = 80
  ...
  third_max_pool = tf.nn.max_pool(third_dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
...
def create_deeep_cnn_model(fingerprint_input, model_settings, is_training):
  """Builds a standard convolutional model.
  ...
  if is_training:
    ...
    second_dropout = second_relu
  second_max_pool = tf.nn.max_pool(second_dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
  third_filter_width = 6
  third_filter_height = 32
  third_filter_count = 80
  third_weights = tf.Variable(
      tf.truncated_normal(
          [
  ...
  fourth_filter_height = 16
  fourth_filter_count = 80
  ...
  fourth_conv = tf.nn.conv2d(third_max_pool, fourth_weights, [1, 1, 1, 1],
                             'SAME') + fourth_bias
  ...
  fourth_max_pool = tf.nn.max_pool(fourth_dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
...
def create_deeeep_cnn_model(fingerprint_input, model_settings, is_training):
  """Builds a standard convolutional model.
  ...
  if is_training:
    ...
    fourth_dropout = fourth_relu
  fourth_max_pool = tf.nn.max_pool(fourth_dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
  fifth_filter_width = 3
  fifth_filter_height = 3
  fifth_filter_count = 256
  ...
  fifth_conv = tf.nn.conv2d(fourth_max_pool, fifth_weights, [1, 1, 1, 1],
                             'SAME') + fifth_bias
  ...
  sixth_filter_width = 3
  sixth_filter_height = 3
  sixth_filter_count = 256
  sixth_weights = tf.Variable(
      tf.truncated_normal(
          [
              sixth_filter_height, sixth_filter_width, fifth_filter_count,
              sixth_filter_count
          ],
          stddev=0.01))
  sixth_bias = tf.Variable(tf.zeros([sixth_filter_count]))
  sixth_conv = tf.nn.conv2d(fifth_dropout, sixth_weights, [1, 1, 1, 1],
                             'SAME') + sixth_bias
  sixth_relu = tf.nn.relu(sixth_conv)
  if is_training:
    sixth_dropout = tf.nn.dropout(sixth_relu, dropout_prob)
  else:
    sixth_dropout = sixth_relu
  sixth_max_pool = tf.nn.max_pool(sixth_dropout, [1, 2, 2, 1], [1, 2, 2, 1], 'SAME')
  sixth_conv_shape = sixth_max_pool.get_shape()
  sixth_conv_output_width = sixth_conv_shape[2]
  sixth_conv_output_height = sixth_conv_shape[1]
  sixth_conv_element_count = int(
      sixth_conv_output_width * sixth_conv_output_height *
      sixth_filter_count)
  flattened_sixth_conv = tf.reshape(sixth_max_pool,
                                     [-1, sixth_conv_element_count])
  label_count = model_settings['label_count']
  final_fc_weights = tf.Variable(
      tf.truncated_normal(
          [sixth_conv_element_count, label_count], stddev=0.01))
  final_fc_bias = tf.Variable(tf.zeros([label_count]))
  final_fc = tf.matmul(flattened_sixth_conv, final_fc_weights) + final_fc_bias
  if is_training:
    return final_fc, dropout_prob
  else:
    return final_fc
